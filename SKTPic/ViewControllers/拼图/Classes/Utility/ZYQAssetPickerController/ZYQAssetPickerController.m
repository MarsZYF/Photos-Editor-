...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (photosSelected && videoSelected)
        format = NSLocalizedString(@"choose %ld projects", nil);
    
    else if (photosSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@"choose %ld photos", nil) : NSLocalizedString(@"choose %ld photos ", nil);
    
    else if (videoSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@"choose %ld videos", nil) : NSLocalizedString(@"choose %ld photos ", nil);
...
- (void)showNotAllowed
{
    ...
    title.text              = NSLocalizedString(@"This application cannot use your photos or video", nil);
...
- (void)showNoAssets
{
    ...
    message.text            = NSLocalizedString(@"You can sync the photos to the iPhone using iTunes for photos and video ", nil);
