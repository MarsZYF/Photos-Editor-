...
- (void)initData
{
    ...
    [[LoadingViewManager sharedInstance] showLoadingViewInView:self.view withText:@"loading..."];
...
- (void)initToolbarView
{
    if (iPhoneX)
    {
        self.bottomControlView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 88-iOS7AddStatusHeight - 33 - 50, self.view.frame.size.width, 50)];
...
//                   forState:UIControlStateNormal];
    ...
    [_posterButton setTag:3];
    [_spliceButton setTag:2];
...
- (void)controlButtonStyleSettingWithButton:(UIButton *)sender
{
    ...
    sender.frame =  CGRectMake(_bottomView.frame.size.width/2.0f*(sender.tag-1), 0, _bottomView.frame.size.width/2.0f, _bottomView.frame.size.height);
...
{
    [[LoadingViewManager sharedInstance] showLoadingViewInView:self.view withText:@"Are dealing with"];
    dispatch_async(dispatch_get_main_queue(), ^{
